...
**Task 3b — Review seam (commits `42be696` + review-trace persistence fix, PILE-02 / Q-14 / Q-08)**
- When `pileModes.review === "live"`, the ModelReviewer is constructed via `createReviewPileModelReviewer({ runPile: <wrapper>, buildMission: () => buildReviewMission(intent, planningAdmission), buildContext: () => ({ provider, signal: runAbortController.signal, budget: resolvePileBudget(...), now }) })`. Falls through to the Phase 5 dry stub or `reviewServices.modelReviewer` when review mode is fixture.
- The `runPile` injected into the reviewer is a thin wrapper that (a) calls `dependencies.runFactoryPile(mission, ctx)` and (b) persists each per-iteration outcome via `writePileArtifacts({ runDir, kind: "review", iteration: N, outcome, refusal? })`. Q-08 (always-persist trace) is honored at every review iteration, not just on the first call. On `outcome.ok=false`, a refusal envelope with `stage: "pile-review"`, `sourceOfTruth: "ReviewPileResult"` is supplied so the iter dir's `refusal.json` carries a typed pile-review refusal.
- Pile failures are also surfaced as a blocking `ModelReviewResult` by `createReviewPileModelReviewer` (with `PileFailure` embedded as a JudgeCritique rationale per Q-12), so the loop terminates with `status=blocked`.
- One integration test: `--review-mode live` with a stubbed `runFactoryPile` that returns a valid `ReviewPileBody` is invoked at least once with `preset.kind === "review"`, and `runs/{id}/piles/review/iter-0/{result.json,trace.json}` exist on disk.
