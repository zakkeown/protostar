...
describe("parseConfirmedIntent schemaVersion 1.3.0 signature envelope", () => {
  it("accepts unsigned 1.3.0 confirmed intents", () => {
    const result = parseConfirmedIntent(buildConfirmedIntentFixture({ signature: null }));

    assert.equal(result.ok, true);
    if (result.ok) {
      assert.equal(result.data.schemaVersion, "1.3.0");
      assert.equal(result.data.signature, null);
    }
  });

  it("accepts signed 1.3.0 confirmed intents with json-c14n@1.0 canonical form", () => {
  ...
  it("rejects legacy 1.0.0 confirmed intents at the parser boundary", () => {
    ...
    if (!result.ok) {
      assert.equal(result.errors.includes('schemaVersion must be "1.3.0" when provided.'), true);
    }
  });
...
function buildConfirmedIntentFixture(overrides: Record<string, unknown> = {}): Record<string, unknown> {
  return {
    ...
    context: "Phase 3 Plan 03 hard-bumps confirmed-intent artifacts to schemaVersion 1.3.0.",
    acceptanceCriteria: [
      {
