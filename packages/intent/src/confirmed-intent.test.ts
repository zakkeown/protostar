...
import { parseConfirmedIntent } from "./confirmed-intent.js";

const VALID_HASH = "0".repeat(64);
const LEGACY_CONFIRMED_INTENT_SCHEMA_VERSION = ["1", "0", "0"].join(".");

describe("parseConfirmedIntent schemaVersion 1.1.0 signature envelope", () => {
  it("accepts unsigned 1.1.0 confirmed intents", () => {
    const result = parseConfirmedIntent(buildConfirmedIntentFixture({ signature: null }));

    assert.equal(result.ok, true);
    if (result.ok) {
      assert.equal(result.data.schemaVersion, "1.1.0");
      assert.equal(result.data.signature, null);
    }
  });

  it("accepts signed 1.1.0 confirmed intents with json-c14n@1.0 canonical form", () => {
  ...
  it("rejects legacy 1.0.0 confirmed intents at the parser boundary", () => {
    ...
    if (!result.ok) {
      assert.equal(result.errors.includes('schemaVersion must be "1.1.0" when provided.'), true);
    }
  });
...
describe("confirmed-intent.schema.json signature envelope", () => {
  it("hard-bumps confirmed-intent schemaVersion and requires canonical sub-hashes", async () => {
    ...
    assert.equal(properties["schemaVersion"]?.["const"], "1.1.0");
...
function buildConfirmedIntentFixture(overrides: Record<string, unknown> = {}): Record<string, unknown> {
  return {
    ...
    context: "Phase 2 Plan 03 hard-bumps confirmed-intent artifacts to schemaVersion 1.1.0.",
    acceptanceCriteria: [
      {
